...
public class GameWonMessage extends GameObject {
    private GameSurface gameSurface=null;
    private int my=0;
    int inc=3;
    static Bitmap createGameWonScreen(GameSurface gameSurface,int y){
        ...
        canvasM.drawText("You Won",widthM/2,heightM/2+y,paint);
    ...
    GameWonMessage(GameSurface gameSurface){
        super(createGameWonScreen(gameSurface,0),1,1,0,gameSurface.getHeight()/3);
    ...
    public void draw(Canvas canvas){
        ...
        if(my<10-(this.getHeight()/3))
            inc = 3;
        my+=inc;
        canvas.drawBitmap(createGameWonScreen(gameSurface,my),x, y, null);
        // Last draw time.

    }
    public GameWonMessage pressed(int x, int y){
